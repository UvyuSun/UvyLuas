...
function user_setup()
    ...
    state.WeaponskillMode:options('Normal','SomeAcc')
...
function init_gear_sets()
    ...
    range={ name="Yoichinoyumi", augments={'DMG:+2',}},
    ammo="Stone Arrow",
    head={ name="Mpaca's Cap", augments={'Path: A',}},
    body={ name="Mpaca's Doublet", augments={'Path: A',}},
    hands={ name="Mpaca's Gloves", augments={'Path: A',}},
		...
		if state.AutoTrustMode.value and not areas.Cities:contains(world.area) and (buffactive['Elvorseal'] or buffactive['Reive Mark'] or not player.in_combat) then
			...
			if party.p5 == nil then
				...
				if spell_recasts[980] < spell_latency and not have_trust("Sylvie") then
					...
					return true
				elseif spell_recasts[952] < spell_latency and not have_trust("Kupofried") then
					windower.chat.input('/ma "Kupofried" <me>')
					tickdelay = os.clock() + 3
					return true
				elseif spell_recasts[967] < spell_latency and not have_trust("Qultada") then
					...
					tickdelay = os.clock() + 3
					return true
				elseif spell_recasts[914] < spell_latency and not have_trust("Ygnas") then
